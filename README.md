...
1)개요 : `JWT token`,`fetch`함수 활용한 서버 인터페이스
    - 결제 : `POST메소드` 통한 마일리지 차감
    - 배송지변경 : `PATCH메소드` 통한 주소 업데잍,
...
3)구현상세
    ...
    - 결제는 POST방식으로 서버통신하여 진행하며 처리결과는 alert로 안내.(재고부족, 잔액부족 이슈 대응 완료)
    - 주소 업데이트는 PATCH방식으로 서버와 통신하여 진행하며 처리결과는 alert로 안내.
